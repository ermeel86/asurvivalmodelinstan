...
Before we dive into the matter, I would like to thank Tiago Caba&ccedil;o and Jacob de Zoete for valuable feedback on an early draft of this case study. I also would like to thank Aki Vehtari, Arya Pourzanjani and Jacki Novik for fruitful discussions on survival models, which will find applications beyond this case study... Stay tuned.
...
```{stan output.var='dump', eval = FALSE, tidy = FALSE}
data {
...
As was pointed out in [this](https://discourse.mc-stan.org/t/survival-models-in-rstanarm/3998/66?u=ermeel) comment on Stan discourse by the user [lcomm](https://discourse.mc-stan.org/u/lcomm/summary), one can directly sample from an exponential distribution with upper bound using the following Stan function^[This needs to be added to functions-block at the beginning of the Stan file.]
...
Our `generated quantities` block would then look as follows
...
and we moved the calculation of `max_time` to the following `transformed data` block:
